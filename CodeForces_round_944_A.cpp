...
// ----------------------</MATH>-------------------------- 
#define FIND_MEX(arr, start, end) ([](const vi& arr,int start, int end) { \
    set<int> nums; \
    int i = start; \
    while (i <= end) { \
  if (arr[i] >= 0) { \
...
#define COPY_AND_FIND_MEX(arr, temp, start, end, mex) \
    do { \
        temp = arr; \
        mex = findMex(temp, start, end); \
    } while (0)
...
{return sqrt(square(x2-x1)+square(y2-y1)+square(z2-z1));} 
 #define INSERT_INTO_SET(arr, nums, start, end) ([](const vector<int>& vec, set<int>& s, int st, int en) { \
    int i = st; \
    while (i <= en) { \
        if (vec[i] >= 0) { \
            s.insert(vec[i]); \
...
#define IS_ALL_ZEROS(arr) ([](const vector<int>& vec) { \
    for (int x : vec) { \
        if (x != 0) return false; \
    } \
    return true; \
...
// Constants
mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
#define CALCULATE_MEX(nums) ([](const set<int>& s) { \
    int mex = 0; \
    while (s.count(mex)) { \
        mex++; \
    } \
...
ll findMex(const vi& arr,int start, int end) {
set<int> nums;
    INSERT_INTO_SET(arr, nums, start, end);
    return CALCULATE_MEX(nums);
}
...
vector<int> arr(n);
    ...
    bool allZeros = IS_ALL_ZEROS(arr);
    if (allZeros) {
        cout << 0 << nl;
        return;
    }
    ...
    while (i < n) {
      ...
      vector<int> temp;
int mex;
            COPY_AND_FIND_MEX(arr, temp, i, j, mex);

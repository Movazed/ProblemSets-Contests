...
void solve() {
    ll startX, startY, startZ;
    ll endX, endY, endZ;
    ll stepSize;
    
    
    INPUT >> startX >> startY >> startZ >> endX >> endY >> endZ >> stepSize;

    
    ll deltaX = abs(endX - startX);
    ll deltaY = abs(endY - startY);
    ll deltaZ = abs(endZ - startZ);

    
    ll maxDelta = MAX3(deltaX, deltaY, deltaZ);
    ll sumDeltas = deltaX + deltaY + deltaZ - maxDelta;

    
    ll blocks = DIVUP(maxDelta, stepSize);

    
    ll totalSteps = (sumDeltas >= blocks - 1) ? (deltaX + deltaY + deltaZ) :
                     ((blocks - 1 - sumDeltas) > 0 && (blocks - 1 - sumDeltas) % 2 == 1) ? 
                     (deltaX + deltaY + deltaZ + blocks - 1 - sumDeltas + 1) :
                     (deltaX + deltaY + deltaZ + blocks - 1 - sumDeltas);

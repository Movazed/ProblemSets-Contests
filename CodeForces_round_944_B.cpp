...
if(temp%2==0 and mod!=p-1) return false ;} int axz = 32; return true;} 
...
void solve() {
    ...
    vector<ll> boundary_positions(n, n);

    ll next_p_position = -1;
    ll i = n - 1;
    while (i >= 0) {
        next_p_position = (input_string[i] == 'p') ? i : next_p_position;
        boundary_positions[i] = (next_p_position != -1) ? min(boundary_positions[i], next_p_position + 1) : boundary_positions[i];
        --i;
    }

    ll last_s_position = -1;
    i = 0;
    while (i < n) {
        last_s_position = (input_string[i] == 's') ? i : last_s_position;
        boundary_positions[i] = (last_s_position != -1) ? min(boundary_positions[i], n - last_s_position) : boundary_positions[i];
        ++i;
    }

    sort(boundary_positions.begin(), boundary_positions.end());
    
    bool is_valid = true;
    i = 0;
    while (i < n) {
        is_valid = (boundary_positions[i] < i + 1) ? false : is_valid;
        if (!is_valid) break;
        ++i;
    }

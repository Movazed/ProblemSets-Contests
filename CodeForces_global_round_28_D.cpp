...
#define CONDITIONAL_PUSH_BACK(arr1, num) \
    if (arr1.empty() || num > arr1[0]) { \
        arr1.push_back(num); \
    }

#define COMPARE_AND_INCREMENT(idx, arr1, arr2, i) \
    while (idx < arr1.size() && arr1[idx] < arr2[i]) { \
        ++idx; \
    }
// -------------------------<RNG>------------------------- 
...
void solve() {
    ...
    std::vector<T> arr1, arr2(y);
    
    int i = 0;
    while (i < x) {
        T num;
        std::cin >> num;
        CONDITIONAL_PUSH_BACK(arr1, num);
    ...
    i = 0;
    while (i < y) {
        COMPARE_AND_INCREMENT(idx, arr1, arr2, i);
        
        (idx > 0 && idx < arr1.size()) ? diff.push_back(arr1.size() - idx) : void();

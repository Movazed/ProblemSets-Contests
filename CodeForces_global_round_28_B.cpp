...
#define ar array
#define ll long long
#define INIT_SMALL_LARGE(small, large, num_elements) \
    small = 1; large = num_elements;

#define INIT_COVERED_VECTOR(covered, num_elements) \
    covered.resize(num_elements + 1, false);

#define PRINT_RESULT(result, i, num_elements) \
    cout << result[i] << (i < num_elements - 1 ? " " : "\n");
...
void solve() {
    ...
    T num_elements, range_size;
    cin >> num_elements >> range_size;
    
    vector<T> positions_to_cover;
    T last_position = 0;
    
    T i = 1;
    while (i <= num_elements - range_size + 1) {
        if (last_position < i) {
            positions_to_cover.push_back(i + range_size - 1);
            last_position = i + range_size - 1;
        }
        i++;
    }
    sort(positions_to_cover.begin(), positions_to_cover.end());
    
    vector<T> result(num_elements, 0);
    

    T small, large;
    INIT_SMALL_LARGE(small, large, num_elements);
    
    vector<bool> covered;
    INIT_COVERED_VECTOR(covered, num_elements);
    

    T index = 0;
    while (index < positions_to_cover.size()) {
        covered[positions_to_cover[index]] = true;
        index++;
    }
    
    i = 1;
    while (i <= num_elements) {
        result[i - 1] = covered[i] ? small++ : large--;
        i++;
    }
    
    i = 0;
    while (i < num_elements) {
        PRINT_RESULT(result, i, num_elements);
        i++;
    }
}

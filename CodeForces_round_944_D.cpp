...
// ----------------------</BITWISE END>-------------------------- 
#define UPDATE_DP_OLD(dp_old, row, x, constant, matrix, cols) \
    dp_old[row][x] = constant * x + matrix[row][(0 + x) % cols]

#define UPDATE_DP_NEW(dp_new, j, x, min_val, constant, value, prev_cost) \
    do { \
        ll cost_from_above= min_val[j] + constant * x + value; \
      ll cost_from_left =prev_cost + value; \
        dp_new[j][x] = std::min(dp_new[j][x], std::min(cost_from_above,cost_from_left)); \
    } while(0)

#define INIT_DP_OLD(dp_old, x, constant, matrix, cols) \
    dp_old[0][x] = constant * x + matrix[0][(0 + x) % cols]
// ----------------------<MATH>--------------------------- 
...
template <typename T>
void update_dp_new_cell(std::vector<std::vector<T>>& dp_new, int j, int x, 
                    const std::vector<T>& min_val, T constant, T value, T prev_cost) {
    T cost_from_above = min_val[j] + constant * x + value;
    T cost_from_left = prev_cost + value;
    dp_new[j][x] = std::min(dp_new[j][x], std::min(cost_from_above,cost_from_left));
}
...
void solve() {
    ...
    std::vector<std::vector<ll>> dp_old(cols, std::vector<ll>(cols, LLONG_MAX));
    std::vector<std::vector<ll>> dp_new(cols, std::vector<ll>(cols, LLONG_MAX));

    int x = 0;
    while (x < cols) {
        INIT_DP_OLD(dp_old, x, constant, matrix, cols);
        x++;
    }
    ...
    while (y < cols) {
        x = 0;
        while (x < cols) {
            dp_old[y][x] = std::min(dp_old[y][x], dp_old[y - 1][x] + matrix[0][(y + x) % cols]);
            x++;
        }
        y++;
    }
    ...
    while (i < rows) {
        int j = 0;
        while (j < cols) {
            std::fill(dp_new[j].begin(), dp_new[j].end(), LLONG_MAX);
            j++;
        }

        std::vector<ll> min_val(cols, LLONG_MAX);
        j = 0;
        while (j < cols) {
            x = 0;
            while (x < cols) {
                min_val[j] = (dp_old[j][x] < min_val[j]) ? dp_old[j][x] : min_val[j];
                x++;}
  j++;
        }
        ...
        while (x < cols) {
            j = 0;
            while (j < cols) {
                ll value = matrix[i][(j + x) % cols];

                UPDATE_DP_NEW(dp_new, j, x, min_val, constant, value, (j == 0) ? LLONG_MAX : dp_new[j - 1][x]);
                j++;
            }
            x++;
        }
    ...
    x = 0;
    while (x < cols) {
        result = std::min(result, dp_old[cols - 1][x]);
        x++;
    }
    std::cout << result << "\n";
}

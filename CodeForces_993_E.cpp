...
#define FOR(currentPower, start, end, factor) for (ll currentPower = start; currentPower <= end; currentPower *= factor)
#define IF(condition) if (condition)
#define ELSE else

// Macro to calculate lowerBound and upperBound
#define BOUNDS(min1, min2, currentPower, max1, max2, lowerBound, upperBound) { \
    lowerBound = max(min1, (min2 + currentPower - 1) / currentPower); \
    upperBound = min(max1, max2 / currentPower); \
}

typedef long long IntervalCount;  // Typedef for count

template <typename T>
void solve(T factor, T min1, T max1, T min2, T max2) {
    IntervalCount count = 0;

    // Loop with macro for iterating over currentPower
    FOR(currentPower, 1, max2 / min1, factor) {
        T lowerBound, upperBound;

        // Use the BOUNDS macro to calculate lowerBound and upperBound
        BOUNDS(min1, min2, currentPower, max1, max2, lowerBound, upperBound);
        ...
        IF (currentPower > max2 / factor) break;
    }
...
int main() {
    ...
    for (int t = 1; t <= tc; t++) {
        ...
        ll max1;
        ll max2;
        cin >> factor >> min1 >> max1 >> min2 >> max2;
        solve(factor, min1, max1, min2, max2);
    }
}

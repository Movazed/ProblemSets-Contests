...
void solve() {
 // implement the logic here
    ...
    for(ll i = 0; i < k1; ++i){
        res = max(res, area(v2[0][0], v2[k2-1][0], v1[i][0], v2[0][1], v2[k2-1][1], v1[i][1]));

    }
    for(ll i = 0; i < k2; ++i){
        res = max(res, area(v1[0][0], v1[k1-1][0], v2[i][0], v1[0][1], v1[k1-1][1], v2[i][1]));
    }
    for(ll i = 0; i < k3; ++i){
        res = max(res, area(v4[0][0], v4[k4-1][0], v3[i][0], v4[0][1], v4[k4-1][1], v3[i][1]));
    }
    for(ll i = 0; i < k4; ++i){
        res = max(res, area(v3[0][0], v3[k3-1][0], v4[i][0], v3[0][1], v3[k3-1][1], v4[i][1]));
    }
    cout << res << nl;

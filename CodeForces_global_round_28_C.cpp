...
#define PRINT_INDICES(a, b, c, d) cout << (a) + 1 << ' ' << (b) + 1 << ' ' << (c) + 1 << ' ' << (d) + 1 << '\n'
#define XOR_ASSIGN(current, index, startPos, inputString, j) (current)[(index) + (startPos)] = ((current)[(index) + (startPos)] - '0') ^ (inputString)[(j)] - '0' + '0'

template <typename T>
void updatePair(pair<T, T>& p, T first, T second) {
    p = {first, second};
}

template <typename T>
int findFirstOccurrence(const T& str, char ch) {
    int i = 0;
...
template <typename T>
bool compareStrings(const T& str1, const T& str2) {
    return str1 > str2;
}
// ----------------------<MATH>--------------------------- 
...
void solve() {
    ...
    INPUT >> inputString;
    string substring;
    pii firstIndex, secondIndex;


    int i = findFirstOccurrence(inputString, '1');
    if (i != -1) {
        substring = inputString.substr(i);
        updatePair(firstIndex, i, (int)inputString.length() - 1);
    }

    if (substring.empty()) {
        PRINT_INDICES(0, 0, 0, 0);
    }
    else {
        int lengthOfSubstring = 0, startPos = 0;


        i = findFirstOccurrence(substring, '0');
        if (i != -1) {
            startPos = i;
            lengthOfSubstring = (int)substring.length() - i;
        }

        if (lengthOfSubstring == 0) {

            i = findFirstOccurrence(inputString, '0');
            if (i != -1) {
                updatePair(secondIndex, i, i);}}
      else {
            updatePair(secondIndex, 0, 0);
            string result = substring;

            i = 0;
            WHILE(i + lengthOfSubstring <= (int)inputString.length()) {
                string current = substring;

                int j = i;
                WHILE(j < i + lengthOfSubstring) {
                    XOR_ASSIGN(current, j - i, startPos, inputString, j);
                    j++;}

                if (compareStrings(current, result)) {
                    result = current;
                    updatePair(secondIndex, i, i + lengthOfSubstring - 1);}
                i++;}}

        PRINT_INDICES(firstIndex.first, firstIndex.second, secondIndex.first, secondIndex.second);
    }
}
